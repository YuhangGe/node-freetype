...
var ttf2woff = require('../generate/ttf2woff.js');
...
Font.prototype._parse = function() {
    ...
    for (var i = 0; i < numTables; i += 1) {
        var tag = parse.getTag(data, p).trim().toLowerCase();
...
//        }
    ...
    if (this._offset['glyf'] && this._offset['loca']) {
        //font file has Trutype Outlines
    } else if (this._offset['cff']) {
        //font file only have PostScript Outlines
        var cff = require('../parse/cff.js');
        //cff.parse(data, this._offset['cff'], this);
    } else {
...
Font.prototype.toWOFF = function() {
    ...
    return ttf2woff(ttf_buffer).buffer;
};
