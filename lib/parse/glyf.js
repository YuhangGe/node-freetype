...
var Glyph = require('../obj/glyph.js');
...
function parseGlyph(data, index, start) {
...
function parseGlyfByIndex(data, start, loca, idx) {
    if(idx >= loca.length -1) {
        throw 'bad index';
    }
    var offset = loca[idx],
        nextOffset = loca[idx+1];
    if(nextOffset > offset) {
        return parseGlyph(data, idx, start + offset);
    } else {
        return new Glyph(idx);
    }
}
...
function parseGlyfTable(data, start, loca) {
    ...
    for (i = 0; i < loca.length - 1; i += 1) {
        offset = loca[i];
        nextOffset = loca[i + 1];
//        console.log('pg: ', offset, nextOffset);
        if (offset !== nextOffset) {
            glyphs.push(parseGlyph(data,  i, start + offset));
        } else {
            glyphs.push(new Glyph(i));
        }
    }
//    console.log(start);
...
exports.parseAllGlyf = parseGlyfTable;
exports.parseGlyfByIndex = parseGlyfByIndex;