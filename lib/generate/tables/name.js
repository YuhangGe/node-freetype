...
function getNames(names) {
    ...
    ['copyright', 'fontFamily', 'uniqueID', 'fontSubfamily', 'licence'].forEach(function(n) {
        if(names[n]) {
            result.push.apply(result, getStrings(names[n], TTF_NAMES[n]));
        }
    });

    result.push.apply(result, getStrings('Generated by node-freetype.', TTF_NAMES.description));
    result.push.apply(result, getStrings('https://github.com/YuhangGe/node-freetype', TTF_NAMES.url));
...
function createNameTable(font) {
    var name_table = font.tables.name;
    var names = getNames(name_table);
